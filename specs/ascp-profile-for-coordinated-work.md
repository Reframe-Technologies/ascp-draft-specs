...
Version: 0.01 — Informational (Pre-RFC Working Draft)  
December 2025
...
This document is part of the ASCP specification suite and defines a **Layer-3 semantic profile** for coordinated work. It specifies two complementary families of **Coordination Constructs**—**Rationale Constructs** and **Work Coordination Constructs**—and a bridging Construct, **Commitment**, that together enable durable, queryable coordination across decisions, goals, tasks, and meetings.
...
This document defines a Layer-3 ASCP profile for capturing that substrate as durable **articulation work**: immutable, addressable, semantically typed Artipoints arranged into an interpretable coordination graph. The profile introduces:
...
- A **Commitment** bridging Construct that converts decisions into executable plans tied to goals and tasks.
...
- what work was committed as a result.
...
ASCP addresses this gap by treating coordination as first-class data: **articulation work made durable**. This profile defines a minimal and interoperable Layer-3 semantics for coordinated work by standardizing:

- the **construct families** and their minimal completeness,
...
A **Rationale Construct** is a Coordination Construct whose primary role is to capture *why* work is done: questions, options, arguments, evidence, decisions, and follow-on intents.

### 4.2 Work Coordination Constructs (WCC)

A **Work Coordination Construct** is a Coordination Construct whose primary role is to capture *what* work is being executed: objectives, goals, tasks, lists, and meeting scaffolding.

### 4.3 Commitment

A **Commitment** is a bridging Construct that converts rationale into execution: it binds a decision to one or more goals and to a set of tasks that implement it.
...
This profile defines a coordinated-work system as three cooperating components:

1. **Rationale layer (RC):** captures decisions and their justification.
2. **Work Coordination layer (WCC):** captures work planning, tracking, and meeting coordination.
3. **Bridge (Commitment):** binds decisions to executable plans.
...
The following Artipoint types are defined by this profile:
...
These types MAY appear as standalone Artipoints or as nodes within Contextual Constructs (Spaces, Streams, Piles) that provide scope.

### 6.2 Denotation and Payload Guidance
...
### 6.3 Link Invariants (Normative)

Within a shared scope:

- A `decision` **MUST** `references` exactly one `issue`.
...
### 6.4 Evolution and Supersession
...
The following WCC Artipoint types are defined by this profile:
...
Optional meeting spine:

- `agenda`
- `discussion_point`

### 7.2 Work Item Proxy Binding (Normative)
...
### 7.3 Decomposition and Organization (Normative)
...
### 7.4 Task Lifecycle and “Current State” (Normative)
...
A Commitment represents an explicit declaration of intent and coordinated plan grounded in a decision and oriented toward one or more goals. It serves as the normalization point where rationale becomes articulated execution context, not a binding obligation, enforceable contract, or protocol-level execution directive.

### 8.3 Link Invariants (Normative)

Within a shared scope:

- A `commitment` **MUST** `references` exactly one originating `decision`.
- A `commitment` **SHOULD** `supports` one or more `goal` Artipoints it advances.
- A `commitment` **MUST** `assembles` (ordered) or `groups` (unordered) the set of `task` Artipoints that implement it.
- A `commitment` **MAY** `assembles` related `action` Artipoints, but actions MUST NOT be treated as the executable work substrate.

### 8.4 Plan Revision

A subsequent `commitment` MAY `replaces` an earlier commitment to represent plan revision. This enables revision of execution plans without rewriting decisions or mutating tasks.
